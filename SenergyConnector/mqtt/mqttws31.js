...
executeFile("userModules/SenergyConnector/mqtt/localStorage.js");
...
//    	    	break;
    ...
    var ClientImpl = function (host, port, clientId) {
        ...
        for (key in localStorage.getMap())
    ...
    ClientImpl.prototype._doConnect = function (host, port) {
        ...
        this.socket = new sockets.websocket(wsurl);
    ...
    ClientImpl.prototype._on_socket_message = function (event) {
        ...
        //decode zway string msg
        var arrayBuffer = new ArrayBuffer(event.data.length * 1);
        var newUint = new Uint8Array(arrayBuffer);
        for(i = 0; i<newUint.length; i++){
            newUint[i] = event.data.charCodeAt(i);
        }

        // Reset the receive ping timer, we now have evidence the server is alive.
        this.receivePinger.reset();
        var byteArray = newUint;
        try {
